...
function create_sample_product() {
	...
	if ( current_user_can( 'publish_products' ) && ((!is_advanced_mode_forced() && empty( $sample_id )) || isset( $_GET[ 'create_sample_product_page' ] )) ) {
		...
		$long_desc .= '<p>' . sprintf( __( 'Thank you for choosing %s. If you have any questions or comments please use <a target="_blank" href="%s">plugin support forum</a>.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://wordpress.org/support/plugin/post-type-x' ) . '</p>';
