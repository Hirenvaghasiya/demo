...
function ic_product_listing_products( $archive_template, $multiple_settings ) {
	...
	if ( (is_tax() || is_search() || !is_ic_only_main_cats() || is_product_filters_active()) && more_products() ) {
		...
		echo '<p>' . sprintf( __( 'Sorry, but nothing matched your search terms. Please try again with some different options or %sreset filters%s.', 'post-type-x' ), '<a href="' . get_filters_bar_reset_url() . '">', '</a>' ) . '</p>';
	}
}
