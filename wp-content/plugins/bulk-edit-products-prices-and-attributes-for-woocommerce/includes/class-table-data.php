...
class Eh_DataTables extends WP_List_Table
{
    ...
    public function input()
    {
        ...
        for ($i = 0; $i < count($product_all_id); $i++) {
            ...
            if ($temp->product_type == 'simple') {
                $meta_thumb                                 = get_post_meta($temp->id, '_thumbnail_id', true);
                $product_data[$i]['product_id']             = $temp->id;
                $product_data[$i]['product_title']          = $temp->post->post_title;
                $product_data[$i]['product_date']           = get_the_date('', $temp->id);
                $product_data[$i]['product_type']           = ucfirst($temp->product_type);
                ...
                $product_data[$i]['product_category']       = $temp->get_categories();
                ...
                $product_data[$i]['product_dimensions']     = ($temp->get_dimensions() != null) ? $temp->get_dimensions() : '-';
    ...
    {
        $per_page              = ($page_count == '') ? 10 : $page_count;
