...
function eh_bep_update_product_callback()
{
    ...
    for ($i = 0; $i < count($product_id); $i++) {
        ...
        if ($temp->product_type == 'simple') {
            ...
            $undo_product_data['title']          = $temp->post->post_title;
            ...
            switch ($sale_select) {
                case 'up_percentage':
                    ...
                    $sale_val = round($undo_product_data['sale'] + $per_val);
                ...
                case 'down_percentage':
                    ...
                    break;
                case 'up_price':
                    $sale_val = round($undo_product_data['sale'] + $sale_text);
                    eh_bep_update_meta_fn($product_id[$i], '_sale_price', $sale_val);
                    break;
                case 'down_price':
                    $sale_val = round($undo_product_data['sale'] - $sale_text);
            ...
            switch ($regular_select) {
                case 'up_percentage':
                    ...
                    $regular_val = round($undo_product_data['regular'] + $per_val);
                ...
                case 'down_percentage':
                    ...
                    break;
                case 'up_price':
                    $regular_val = round($undo_product_data['regular'] + $regular_text);
                    eh_bep_update_meta_fn($product_id[$i], '_regular_price', $regular_val);
                    break;
                case 'down_price':
                    $regular_val = round($undo_product_data['regular'] - $regular_text);
    ...
    for ($i = 0; $i < count($all_id); $i++) {
        ...
        if ($temp->product_type == 'simple') {
            ...
            if ($product_category_valid != 0 && $attribute_valid != 0 && $attribute_value_valid != 0 && $price_valid != 0) {
                array_push($filtered_id, $temp->id);
            }
        }
    }
    session_start();
