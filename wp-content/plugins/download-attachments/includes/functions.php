 ...
 * @return	integer					Numer of attachment downloads
 */
	...
	function da_most_downloaded_attachments( $args = array(), $display = true ) {
		...
		if ( ! empty( $attachments ) ) {
			...
			foreach ( $attachments as $attachment ) {
				...
				$file = wp_check_filetype( get_attached_file( $attachment->ID ) );
				...
				$html .= '
					<a class="post-title" href="' . ( $args['link_type'] === 'page' ? get_permalink( $attachment->ID ) : ( $options['pretty_urls'] === true ? home_url( '/' . $options['download_link'] . '/' . $attachment->ID . '/' ) : DOWNLOAD_ATTACHMENTS_URL . '/includes/download.php?id=' . $attachment->ID ) ) . '">' . esc_html( get_the_title( $attachment->ID ) ) . '</a>' . ( $args['show_attachment_downloads'] ? ' <span class="count">(' . number_format_i18n( da_get_attachment_downloads( $attachment->ID ) ) . ')</span>' : '' );
